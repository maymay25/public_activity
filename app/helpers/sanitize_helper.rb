module SanitizeHelper
	...
	def sanitize_content(content)
		...
		Sanitize.clean(content, :elements => ['embed','p','img','pre','b','strong','i','em','blockquote','sup','span'],
		    :attributes => {'embed' =>['type','pluginspage','src','width','height','edui-faked-video','wmode','play','loop','align','menu','allowscriptaccess','allowfullscreen'],
	...
	def sanitize_embed_summary(content)
		...
		transformer = lambda do |env|
		  ...
		  Sanitize.clean_node!(node, {:elements => ['embed'],
							:attributes => {'embed' =>['type','pluginspage','src','width','height','edui-faked-video','wmode',
														    				'play','loop','align','menu','allowscriptaccess','allowfullscreen']}})
		  {:node_whitelist => [node]}
		end
